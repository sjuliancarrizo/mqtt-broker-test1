  ...
  if (ev == MG_EV_MQTT_CMD) {
    ...
    switch (mm->cmd) {
      case MQTT_CMD_CONNECT: {
        ...
        } else if (mm->payload.p[8] != 4) {
          mg_error(c, "Unsupported MQTT version %d", mm->payload.p[8]);
        } else {
      ...
      case MQTT_CMD_SUBSCRIBE: {
        ...
        while ((pos = mg_mqtt_next_sub(mm, &topic, &qos, pos)) > 0) {
          ...
          MG_INFO(
              ("SUB %p [%.*s]", c->fd, (int) sub->topic.len, sub->topic.p));
      ...
      case MQTT_CMD_PUBLISH: {
        ...
        MG_INFO(("PUB %p [%.*s] -> [%.*s]", c->fd, (int) mm->data.len,
                 mm->data.p, (int) mm->topic.len, mm->topic.p));
    ...
    for (struct sub *next, *sub = s_subs; sub != NULL; sub = next) {
      next = sub->next;
      if (c != sub->c) continue;
      MG_INFO(("UNSUB %p [%.*s]", c->fd, (int) sub->topic.len, sub->topic.p));
    ...
    switch (mm->cmd) {
      ...
      case MQTT_CMD_SUBSCRIBE: {
        ...
        while ((pos = mg_mqtt_next_sub(mm, &topic, &qos, pos)) > 0) {
          ...
          sub->qos = qos;
          LIST_ADD_HEAD(struct sub, &s_subs, sub);
          //MG_INFO(("SUB %p [%.*s]", c->fd, (int) sub->topic.len, sub->topic.p));
          // Change '+' to '*' for topic matching using mg_match
          for (size_t i = 0; i < sub->topic.len; i++) {
            if (sub->topic.p[i] == '+') ((char *) sub->topic.p)[i] = '*';
          }
